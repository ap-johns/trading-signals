...
ETP_VERDICTS = {
    "suitable": ("Suitable", "#00e676", "2x beat the shares in most 18-month holds and rarely halved"),
    "marginal": ("Marginal", "#f0d060", "mixed record: some edge, real chance of a large loss"),
    "avoid":    ("Avoid",    "#ff5252", "daily reset feeds on this name's volatility; lost half in many windows"),
    "thin":     ("No history", "#888",  "under ~2 years of rolling windows; vol only"),
}
LEAP_TIER_LABEL = {
...
def leap_section_html(all_data, config):
    ...
    for cat in cfg.get("categories", []):
        ...
        for yf_ticker, name in tickers.items():
            ...
            etp = all_data.get(yf_ticker, {}).get("etp")
            etp = None if (etp is None or "error" in etp) else etp
            items.append({
                "name": name, "sector": sectors.get(name), "tier": tier, "snap": snap,
                "flag": leapmod.leap_flag(tier, snap), "iv_rank": rank, "iv_n": n_obs,
                "etp": etp, "etp_verdict": leapmod.etp_verdict(etp),
            })
    if not items:
        return ""
    ...
    def etp_tick(it):
        """London 2x ticker label for a name: 'NVD2 / 2NVD', '3x only', or None."""
        tk = leapmod.ETP_TICKERS.get(it["name"])
        if tk:
            return " / ".join(t for t in tk[:2] if t)
        if it["name"] in leapmod.ETP_NOTES:
            return "3x only"
        return None
    ...
    def strong_entry(it):
        """Strong-setup name plus a 2x ETP verdict: the London ticker in green
        when suitable, amber when marginal, a red 'avoid 2x' when the daily
        reset would likely lose to the shares, grey when no product exists."""
        tick, v = etp_tick(it), it.get("etp_verdict")
        tip = etp_tip(it)
        if v == "avoid":
            tag = '<span style="color:#ff5252;">avoid 2x' + (f' {tick}' if tick else "") + '</span>'
        elif v == "suitable":
            tag = f'<span style="color:#00e676;">2x ok' + (f' {tick}' if tick else "") + '</span>'
        elif v == "marginal":
            tag = f'<span style="color:#f0d060;">2x marginal' + (f' {tick}' if tick else "") + '</span>'
        else:
            tag = '<span class="fib-dt">2x untested' + (f' {tick}' if tick else "") + '</span>'
        return f'<span title="{tip}">{it["name"]} <span class="fib-dt">(</span>{tag}<span class="fib-dt">)</span></span>'

    strong = [strong_entry(it) for it in items if it.get("flag") == "strong"]
    etp_ok = [f'<span title="{etp_tip(it)}">{it["name"]}</span>' for it in items if it.get("etp_verdict") == "suitable"]
    etp_no = [f'<span title="{etp_tip(it)}">{it["name"]}</span>' for it in items if it.get("etp_verdict") == "avoid"]
    summary = (f'<div class="fib-summary">'
               ...
               f'<span class="fib-dt">&mdash; favoured tier + fair premium; the grade to wait for. Bracket = whether a 2x ETP is a sound alternative, with its London ticker</span></div>'
               f'<div class="fib-sum-line"><span class="fib-sum-tag fav">2x ETP suitable</span> {", ".join(etp_ok) or "none"} '
               f'<span class="fib-dt">&mdash; tax-free leverage inside an ISA; hover a name for the 18-month test</span></div>'
               f'<div class="fib-sum-line"><span class="fib-sum-tag skip">2x ETP avoid</span> {", ".join(etp_no) or "none"} '
               f'<span class="fib-dt">&mdash; daily reset feeds on volatility; halved in many 18-month holds</span></div></div>')
...
def main():
    ...
    for yf_ticker, (display_name, category) in all_tickers.items():
        ...
        if category in leap_categories:
            ...
            try:
                full = yf_history(yf.Ticker(yf_ticker), period="max", interval="1d")
                d["etp"] = leapmod.etp_stats(full["Close"]) if not full.empty else None
            except Exception as e:  # noqa: BLE001
                d["etp"] = {"error": str(e)}
            if price:
                try:
