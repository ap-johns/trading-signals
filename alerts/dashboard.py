...
def leap_section_html(all_data, config):
    ...
    def strong_entry(it):
        """Strong-setup name with its London 2x ticker where one exists."""
        tick = etp_tick(it)
        return f'{it["name"]}' + (f' <span class="fib-dt">({tick})</span>' if tick else "")
    ...
    summary = (f'<div class="fib-summary">'
               ...
               f'<span class="fib-dt">&mdash; favoured tier + fair premium; the grade to wait for. 2x ETP ticker in brackets where London lists one</span></div>'
